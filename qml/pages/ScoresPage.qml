...
Page {
    ...
    SilicaListView {
        ...
        function refresh() {
            ...
            xhr.open('GET', 'https://liiga.fi/game-tracking/' + date.getFullYear().toString() + '/' + ('0' + (date.getMonth() + 1).toString()).substr(-2) + '/' +  ('0' + date.getDate().toString()).substr(-2) + '/0?' + new Date().getTime());
            xhr.send();
        }
